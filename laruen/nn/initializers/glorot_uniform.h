...
namespace laruen::nn::initializers {
    ...
    namespace impl {
        ...
        class GlorotUniform : public Initializer<T> {
            ...
            public:
                inline GlorotUniform(std::mt19937::result_type seed = std::random_device{}(),
        ...
        template <typename T = float32_t>
        inline std::shared_ptr<Initializer<T>> glorot_uniform(
            std::mt19937::result_type seed = std::random_device{}(), uint_fast64_t count = 0) noexcept
        {
            return std::shared_ptr<Initializer<T>>(new GlorotUniform<T>(seed, count));
        }
    }
