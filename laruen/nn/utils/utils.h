...
namespace laruen::nn::utils {
    ...
    namespace impl {
        ...
        template <typename T>
        inline const NDArray<T> batch_view(const NDArray<T> &ndarray, uint_fast64_t batch_size) {
        ...
        template <typename T>
        inline NDArray<T> batch_view(NDArray<T> &ndarray, uint_fast64_t batch_size) {
